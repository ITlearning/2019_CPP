...
public:
	...
	void cheerup(Participant p[]) {
		...
		for (int i = 0; i < People; i++) {
			if (p[i].getName() == name) {
				cout << "자신의 이름과,하고 싶은 말을 남기세요!" << endl;
				cout << ">> ";
				cin >> username >> commit;
...
public:
	...
	void show(Participant p[]) {
		cout << "현재 프로그램에 참여하고 있는 연습생 목록" << endl;
		for (int i = 0; i < People; i++) {
			cout <<"["<< i + 1 <<"] "<< " : " << p[i].getName() << " [파트]: "<< p[i].getPosition() << " [키]: " << p[i].getTall() << " [나이]: " << p[i].getAge() << " [몸무게]: " << p[i].getWeight() << endl;
		}
	}
	...
	void comment(Participant p[]) {
		...
		cout << "글들을 보기 원하시는 참가자의 이름을 입력하세요" << endl;
		for (int i = 0; i < People; i++) {
			cout << p[i].getName() << ' ';
		}
		cin >> name;
